...
class FinishAJob(unittest.TestCase):
    ...
    def _test_finish_job_request(self, data, expected_response, expected_params=None, expected_stat=8):
        with self.app as client:
            ...
            if self._testMethodName == "test_start_job_successfully":
                stat = AutoServiceJob.query.filter(AutoServiceJob.id == data[Keys.JOB_ID]).with_entities(AutoServiceJob.status_id).first()[0]
                self.assertEqual(expected_stat, stat)

    def test_finish_job_successfully(self):
        # Done
        data = {
            Keys.JOB_ID: 1,
            Keys.USER_TYPE: Keys.FINISH_JOB_REQUEST
        }
        self._test_finish_job_request(data, 200)
    ...
    def test_wrong_status_type(self):
        ...
        self._test_finish_job_request(data, 404, None)
    ...
    def test_no_db(self):
        ...
        self._test_finish_job_request(data, 500, 6)

    def test_bad_schema(self):
        data = {
            Keys.BUSINESS_OWNER_ID: 'should be integer'
        }
        self._test_finish_job_request(data, 400, 5)
