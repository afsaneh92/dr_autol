...
from core.validation.helpers import password_regex_checker, phone_number_regex_checker, name_regex_checker


class AdminRegistrationRequest(RequestBaseClass):
    ...
    def validate_pattern(self):
        ...
        if not result[0]:
            invalid_params.append(result[1])
        result = self._name_checker(self.last_name)
        if not result[0]:
            invalid_params.append(result[1])
    ...
    def pre_deserialize(json_dict):
        ...
        if 'last_name' not in json_dict:
            missed_params.append("missing_last_name_in_json")
