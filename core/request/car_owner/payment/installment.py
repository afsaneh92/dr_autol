...
from persistence.database.entity.job_ import Job


class InstallmentRequest(RequestBaseClass):
    def __init__(self, json_obj):
        self.job = None
        self.json_obj = json_obj

    def validate_pattern(self):
        if len(self.job.name.strip()) == 0:
    ...
    def pre_deserialize(json_dict):
        ...
        """
        missed_params = []
        if 'installments' not in json_dict:
            missed_params.append("no installments")
    ...
    def deserialize(self):
        ...
        result = InstallmentRequest.pre_deserialize(json_dict)
        if not result[0]:
            return result
        job = Job(id=json_dict[Keys.JOB_ID])
        self.job = job
