...
class CodeValidationController(BaseController):
    ...
    def execute(self):
        result = self.is_registered_before()
        if not result[0]:
    ...
    def _update_user(self, user):
        ...
        if result[0]:
            success = SuccessValidation(status=200, message="%s aziz, mamnon.s" % self.code.phone_number, params=None)
            return True, success
        return result
