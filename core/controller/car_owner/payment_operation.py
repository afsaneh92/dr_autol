...
class PaymentOperationController(BaseController):
    ...
    def _push_notification_done_payment(self):
        ...
        loaded_business_owner_result = BusinessOwners.find(id=self.job.business_owner_id)
    ...
    def do_payment_operation(self):
        ...
        try:
            ...
            if not error_free:
                raise Exception()
            db.session.commit()
            result = True, SuccessPaymentAdded(status=200, message=Result.language.PAYMENT_PAID_SUCCESSFULLY, params=None)
        except:
            db.session.rollback()
    ...
    def _is_amount_enough(self):
        error_free, job_price = calculate_job_payment_price(self.job)

        if not error_free:
            return False, job_price
